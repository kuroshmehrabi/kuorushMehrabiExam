...
namespace Library.Services.ServiceClasses
{
    public class LibraryService : ILibraryService
    {
        ...
        public async Task<LibraryDTO> MapToDTO(Libraryy library)
        {
        ...
        public async Task<Libraryy> MapToEntity(LibraryDTO library)
        {
            return await Task.Run(() => new Libraryy()
            {
                Id = library.Id,
                Address = library.Address,
                EmailAddress = library.EmailAddress,
        ...
        {
            IQueryable<Libraryy> datas = await _unitOfWork.LibraryRepository.GetAll();
        ...
        public async Task<IEnumerable<LibraryDTO>> GetAllLibraryWithPagination(string sortOrder, string currentFilter, string searchString, int? page, int? pageSize)
        {
            ...
            return libraries != null && libraries.Any() ? libraries.Select(x => MapToDTO(x).Result).AsEnumerable() : null;
        }
    }
}
